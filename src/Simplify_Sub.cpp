    ...
    if (may_simplify(op->type)) {
        ...
        if (EVAL_IN_LAMBDA
            ((!op->type.is_uint() && 
             ...
             rewrite(select(x, y, z) - (w + select(x, u, v)), select(x, y - u, z - v) - w) ||
             rewrite((select(x, y, z) - w) - select(x, u, v), select(x, y - u, z - v) - w) ||
             (!exclude_invalid_ordering_rules && rewrite(c0 - select(x, c1, c2), select(x, fold(c0 - c1), fold(c0 - c2)))) ||
             ...
             (!exclude_invalid_ordering_rules && rewrite((c0 - x) - (y + c1), fold(c0 - c1) - (x + y))) ||
             ...
             (no_overflow_int(op->type) &&
              ((!exclude_invalid_ordering_rules && rewrite(c0 - (c1 - x)/c2, (fold(c0*c2 - c1 + c2 - 1) + x)/c2, c2 > 0)) ||
               (!exclude_invalid_ordering_rules && rewrite(c0 - (x + c1)/c2, (fold(c0*c2 - c1 + c2 - 1) - x)/c2, c2 > 0)) ||
