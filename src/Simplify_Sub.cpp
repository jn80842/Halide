...
Expr Simplify::visit(const Sub *op, ExprInfo *bounds) {
    ...
    if (may_simplify(op->type)) {
        ...
        if (EVAL_IN_LAMBDA
            ((!op->type.is_uint() && 
              (!exclude_misordered_rules && rewrite(x - c0, x + fold(-c0), !overflows(-c0)))) ||
             ...
             (!exclude_misordered_rules && rewrite(x - y*c0, x + y*fold(-c0), c0 < 0 && -c0 > 0)) ||
             ...
             (no_overflow_int(op->type) &&
              (rewrite(c0 - (c1 - x)/c2, (fold(c0*c2 - c1 + c2 - 1) + x)/c2, c2 > 0) ||
               ...
               (!exclude_misordered_rules && rewrite(((x + y) + z)/c0 - ((y + x) + w)/c0, ((x + y) + z)/c0 - ((x + y) + w)/c0, c0 > 0)) ||
