...
//    const Scope<Expr> &external_scope;
    ...
    Expr visit(const Sub *op) override {
        ...
        if (rewrite(0 - tarv0, 0 - tarv0, "negate_escape_hatch")) {
            return a - b;
        }
...
SolverResult solve_expression(const Expr &e, const std::string &variable, const Scope<Expr> &scope) {
    // debug(0) << "Calling SolveTRS on " << e << " for target var " << variable << "\n";
    SolveTRSExpression trs_solver(variable);
    Expr intermediate_e = trs_solver.mutate(e);
    // debug(0) << "Called SolveTRS on " << e << " got " << intermediate_e << " for target var " << variable << "\n";
    // debug(0) << "Calling Solve on " << e << " for target var " << variable << "\n";
